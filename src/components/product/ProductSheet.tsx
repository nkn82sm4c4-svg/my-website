...
export function ProductSheet() {
  ...
  return (
    <ProductSheetBody
      key={`${productSheet.productId}:${productSheet.view}`}
      productId={productSheet.productId}
      initialView={productSheet.view}
    />
  )
}
...
function ProductSheetBody({ productId, initialView }: { productId: string; initialView: 'photo' | '3d' }) {
  ...
  const toggle = (a: AddOn) => setAddOns((cur) => (cur.some((x) => x.id === a.id) ? cur.filter((x) => x.id !== a.id) : [...cur, a]))

  return (
    <Sheet open onClose={closeProduct} bare size="lg">
      {/* phones: stacked • desktop: 3D/photo on one side, details on the other */}
      <div className="md:grid md:grid-cols-[1.15fr_1fr]">
        <div className="md:sticky md:top-0 md:flex md:h-[88dvh] md:flex-col md:bg-gradient-to-b md:from-[#fff3e2] md:to-sand md:pb-6">
          {/* media */}
          <div className="relative -mt-7 h-[min(46dvh,380px)] bg-gradient-to-b from-[#fff3e2] to-cream md:mt-0 md:h-auto md:flex-1 md:bg-none">
            ...
            <button
              ...
              aria-label="إغلاق"
              className={cx(
                'tap absolute z-10 grid size-9 place-items-center rounded-full bg-white/90 shadow-sm backdrop-blur md:hidden',
          ...
          {product.model && (
            <div className="relative z-10 -mt-5 flex justify-center md:mt-0">
        ...
        <div className="md:flex md:min-h-[88dvh] md:flex-col">
          <div className="space-y-5 px-5 pt-4 pb-32 md:flex-1 md:px-7 md:pt-7 md:pb-8">
            <div className="hidden justify-end md:flex">
              <button onClick={closeProduct} aria-label="إغلاق" className="tap grid size-9 place-items-center rounded-full bg-sand">
                <X className="size-4" />
              </button>
            </div>
            <div>
          ...
          {/* sticky CTA */}
          <div className="sticky bottom-0 z-20 flex items-center gap-3 border-t border-line bg-cream/95 px-5 pt-3 pb-safe backdrop-blur md:px-7 md:pb-5">
